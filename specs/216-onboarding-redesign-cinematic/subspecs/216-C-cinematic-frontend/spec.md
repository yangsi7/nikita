...
|----|-------------|----------|
| **C1.1** | All 15 base wizard screens implemented matching `wireframes/ascii.md` + `wireframes/figma.md` (welcome + 11 visual slot screens + backstory pick + phone + completion; `together_we_could` and `same_weird_if` slots collected from one combined screen with two textareas per C1.18). Visual diff against Figma frames ≤2% pixel drift on mobile + desktop. | HIGH |
...
| **C1.18** | **Wizard total = 15 screens** (welcome + 11 visual slot screens + backstory pick + phone + completion). Slot count remains 12 (B1.1 unchanged): `together_we_could` and `same_weird_if` are both populated from ONE combined screen with two textareas (headline `T O G E T H E R   /   O D D`), per CRO free-text-fatigue mitigation. Both slots remain individually validated by FinalForm (B1.2). Resume mid-wizard restores to last incomplete slot per C1.15. | HIGH |
