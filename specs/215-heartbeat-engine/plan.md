...
- **Acceptance Criteria** (UPDATED iter-2 per GATE 2 fixes H-5, H-7, H-8, H-10, M-12, M-13, M-15):
  ...
  - **AC-T2.1-007** (H-10 fix; UPDATED post-QA-iter-1 PR #326): The composite unique index `idx_nikita_daily_plan_user_date` (AC-T2.1-002) ALREADY serves the cron's "today's plan" lookup efficiently via the leading `user_id` column — Postgres can satisfy `WHERE user_id = ? AND plan_date = ?` from this composite. A separate partial index on `plan_date` with `WHERE plan_date >= CURRENT_DATE - INTERVAL '1 day'` is NOT expressible because Postgres requires IMMUTABLE predicates and `CURRENT_DATE` is STABLE; a non-partial `plan_date`-only index would be redundant with the composite. **Decision**: drop the proposed `idx_nikita_daily_plan_recent` index entirely; the composite + idx_nikita_daily_plan_generated_at (AC-T2.1-008) cover the access patterns. Migration comment block documents this decision inline for traceability.
