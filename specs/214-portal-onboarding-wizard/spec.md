...
- **AC-9.6 (GH #313 regression guard, added 2026-04-17):** CTA click MUST call `patchProfile({location_city, social_scene, drug_tolerance, life_stage, interest, name, age, occupation, wizard_step: 8})` BEFORE `selectBackstory(chosen_option_id, cache_key)`, and MUST `await` the PATCH to resolution before issuing the PUT. Rationale: the backend's clearance check recomputes `compute_backstory_cache_key` from `users.onboarding_profile` JSONB (see AC-10.3) and rejects when the recomputed key does not match the submitted one. Without a synchronous PATCH-before-PUT, the JSONB stays empty, the server recomputes `unknown|unknown|unknown|...`, and every fresh user gets a `403 "Clearance mismatch. Start over."` blocking onboarding at step 6/7. If `patchProfile` rejects, `selectBackstory` MUST NOT be called and `onAdvance` MUST NOT fire — re-enable the CTA so the user can retry without a half-persisted race. Falsifiable test: `BackstoryReveal.test.tsx` asserts (i) `patchProfile` is called with the full collected profile, (ii) order is `patch → select` via a `callOrder` array, (iii) a rejected `patchProfile` leaves `selectBackstory` uncalled and the CTA re-enabled.
