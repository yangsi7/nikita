...
| AC-2.1bis (FE-M1) | Tap surface MUST render as a native `<button>` element (GlowButton wraps `<button>`, not `<div onClick>`). Vitest assertion: `expect(screen.getByRole("button", { name: /tap to enter/i })).toBeInTheDocument()`. Keyboard activation MUST work: `onKeyDown={(e) => { if (e.key === "Enter" \|\| e.key === " ") handleAdvance(); }}` (GlowButton handles natively if true `<button>`). Vitest keyboard-only path: `await user.tab(); await user.keyboard("{Enter}");` advances to `/onboarding`. Tap surface size: full viewport (100vw × 100vh) for tap-anywhere ergonomics; inner GlowButton CTA: ≥44×44px touch target, max-width 320px centered (WCAG 2.1 AA touch-target compliance). | HIGH |
...
| AC-2.4 (AUTH-M2 RESOLVED) | UA detection MUST happen SERVER-SIDE in the `interstitial/page.tsx` Server Component using `userAgent({ headers: await headers() })` from `next/server` (canonical pattern for Next.js 16 RSC). Pass `requireGesture: boolean` as a prop to `InterstitialClient`. NEVER use `window.navigator.userAgent` (regression source — silent SSR/CSR mismatch). Code shape: `import { headers } from "next/headers"; import { userAgent } from "next/server"; const ua = userAgent({ headers: await headers() }); const isIOSWebKit = ua.os.name === "iOS" && ua.browser.name === "Mobile Safari"; const isTelegramIAB = ua.ua.includes("Telegram"); const requireGesture = isIOSWebKit \|\| isTelegramIAB \|\| /* unknown UA fallback */ true; return <InterstitialClient requireGesture={requireGesture} next={searchParams.next} />;` | HIGH |
| AC-2.4bis | Vitest verifies SSR-only UA detection: server-rendered HTML for an iOS UA contains `data-require-gesture="true"`; for a desktop Chrome UA contains `data-require-gesture="false"`. NO `window.navigator` call permitted in InterstitialClient.tsx — verified by `grep -n "navigator.userAgent" portal/src/app/auth/interstitial/` returning empty. | MEDIUM |
