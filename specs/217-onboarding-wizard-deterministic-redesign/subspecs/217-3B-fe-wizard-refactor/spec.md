...
| AC-11.4 (FE-M2 RESOLVED) | Both regions use shadcn/ui `Card` + framer-motion `AnimatePresence` (opacity+y+blur) per Spec 208 design system. Responsive layout MUST be specified explicitly across breakpoints to avoid mobile/desktop drift: **Mobile (<640px)**: `<main class="flex flex-col gap-4 p-4"> <DeterministicTrack class="w-full" /> <AgentSubspace class="w-full" /> </main>` (vertical stack, full-width). **Tablet (640-1023px)**: same vertical stack, max-width 480px, centered (`mx-auto`). **Desktop (≥1024px)**: same vertical stack centered (no side-by-side per Spec 208 single-column 12-screen wizard — sibling DOM is logical, not visual side-by-side). **Touch targets**: every interactive element ≥44×44px (WCAG 2.1 AA). **IdentityPair card**: max-width 480px, padding 24px, single submit button bottom-right. Verification: vitest `WizardShell.test.tsx` snapshots at 360px, 768px, 1280px viewport widths assert `getComputedStyle().flexDirection === "column"` at all three; Playwright e2e walk B3 captures screenshots at the same three breakpoints. | HIGH |
