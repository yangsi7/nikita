...
class UserProfile(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        # QA iter-5 F1 (PR #282): mirror the migration's age CHECK at the ORM
        # layer so DDL generated by ``metadata.create_all()`` matches the
        # production migration. CheckConstraint in __table_args__ emits a
        # ``CHECK`` in the CREATE TABLE SQL — it does NOT perform Python-side
        # validation; ``UserProfile(age=200)`` instantiates cleanly in-memory
        # and only the DB (via INSERT/UPDATE) rejects the value. This is
        # consistent with how ``check_drug_tolerance_range`` above works.
