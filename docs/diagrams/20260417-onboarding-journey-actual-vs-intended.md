...
| Telegram `/start` no-payload routing | Portal bridge button for every branch (new/game_over/limbo/pending) | Bridge nudge with portal CTA across all branches; legacy Q&A removed | **FIXED** | **#357 (FR-11c)** |
| Bot message_handler pre-onboard text | Bridge nudge; no Q&A advance | Pre-onboard text routed to portal bridge; no Q&A advance | **FIXED** | **#357 (FR-11c)** |
