...
| 217 | onboarding-wizard-deterministic-redesign | #560 #562 #563 #565 #572 (merged) | **SUPERSEDED BY SPEC 218 (2026-05-09)** — Walk B5 PARTIAL verdict surfaced 4 cascading defects (stale cross-slot reactions, progressbar pinned at 7%, backstory pipeline timeout, backarrow chrome collision). Architectural emission-union pattern bulldozed. PRs shipped on master `7ae1189`; lifecycle marked superseded at spec-authoring time per `.claude/rules/doc-lifecycle.md`. |
| 218 | onboarding-wizard-v2-agent-driven | 0 (NEW) | **COMPLETE (PR-218-8, 2026-05-13)** — supersedes 217. Hybrid agent-driven dynamic-UI: Python router picks slot deterministically; Pydantic AI decorator agent emits 8-component discriminated-union envelope per turn (text_short, text_long, single_select, chip_multi, slider, calendar, phone, complete) decorated with reaction text folded into prompt; Phase 1 required slots + Phase 2 open-bounce 4-8 turns. Wow features: phone-demo opt-in via `voice_service.make_outbound_call` + Supabase Realtime + full-screen takeover. Static cohort lookup (NOT real-time firecrawl) for personalized hangouts. ~3000 LOC across 8 PRs (218-PREREQ-A backstory-timeout fix + 218-1..218-7 + 218-8 v1 bulldoze + flag flip). 25 architectural decisions locked in `~/.claude/plans/immutable-wondering-gray.md` brief; 8 reviewers verified (Pydantic AI research, Gemini deep research, process-auditor 31/40, devils-advocate, scope-reviewer, pattern-scout, codex, gemini reviewer). `wizard_v2_enabled` flipped to `True` (PR-218-8). Solo-dev no-backcompat per `feedback_solo_dev_no_backcompat_reinforced.md`. |

**Domain subtotal: 19 specs (16 complete, 3 active — Specs 216, 217 superseded-by-218, 218 COMPLETE)**
...
**Spec 216 — onboarding-redesign-cinematic** (ACTIVE, audit re-pass pending). Master spec.md + 8 subspecs (216-A..216-F + 216-G + 216-H). Subspec PRs merged on `feat/216-*` branches: #450 (SDD Phase 1 + 216-A backend routing), #452 (216-B1+B2 atomic agentic-wizard-core rewrite), #457 (216-B3 POST /answer + GET /state + flag-gated /converse 410 shim), #461 (216-D-code Big5 judge + 12 archetypes + cohort chips), #462 (216-E firecrawl tools + WebSearchTool + cost guard), #464 (216-C cinematic 15-screen wizard + auth-guarded resume), **#537 (216-G TG-first canonical landing CTAs + delete `/onboarding/auth` page-client, 2026-05-06)**, **#538 (216-H autobind fail-closed + 410 GONE stub + Vercel project relink — PR-E, 2026-05-06)**, **#539 (216-H/F HOTFIX env.ts must use static `process.env.X` access, 2026-05-06)**. audit-report.md FAIL 2026-04-30 (4 CRIT + 8 HIGH + 5 MED) — re-audit pending after subspec merges. Supersedes Spec 214 v2 FR-11d (GH #396 to be closed by 216-B) and Spec 215A portal-first auth chain (216-A rewires bare `/start` for unbound users from `_handle_start` deep-link to `SignupHandler.handle_welcome` FSM). 10 W3 findings (#440-#449) all mapped to subspec ACs. Wireframes: ASCII (127K) + Figma file (20 frames) live. Cost ceiling $0.50/flow, latency p99 8s/turn.
