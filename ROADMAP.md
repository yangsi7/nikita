...
| Last deploy | 2026-04-19 (PR #368, FR-11e ceremonial handoff T4.1-T4.5; merged 8fbcf57) |
| Active specs | 1 (214 portal-wizard — FR-1..11b + FR-11c + FR-11d + FR-11e (T4.1-T4.5) shipped; T4.6-T4.8 deferred) |
| In-flight | Spec 214 PR 5 (legacy cleanup, deferred ≥7 days post-chat-first stabilization) |
...
| 214 | portal-onboarding-wizard | 165+ | **IN PROGRESS, AMENDED 2026-04-19**. FR-1..FR-11b shipped (214-A/B/C/D + entry #312 + #315/#317/#319 JSONB fixes + #322 Telegram binding). **FR-11c SHIPPED 2026-04-19 (PR #357, merged 371fb97)** — Telegram `/start` and pre-onboard free-text now route to portal bridge; legacy in-bot Q&A fully eliminated for unonboarded/game_over/limbo branches; bridge tokens minted via `telegram_link_repository`. **FR-11d backend SHIPPED 2026-04-19 (PR #361, merged 4b298f6)** — T2.1-T2.11: tuning constants + extraction schemas + ConverseRequest/Response + conversation agent with persona-imported-verbatim prompt + /converse endpoint with rate limits/idempotency/spend cap + JSONB persistence + handoff greeting + persona-drift ADR + measurement script. 165 new tests, full suite green (6478 passing). Supabase migrations `llm_idempotency_cache` + `llm_spend_ledger` applied post-merge via MCP (RLS enforced; pg_cron scheduling deferred — service role lacks cron permission, needs manual cron registration). **FR-11d frontend SHIPPED 2026-04-19 (PR #363, merged 4c02459)** — chat-first wizard UI with message-bubble thread, typewriter reveal, typing indicator, inline controls (text/chips/slider/toggle/cards), confirmation buttons, progress header, ceremony gate. **FR-11e SHIPPED 2026-04-19 (PR #368, merged 8fbcf57)** — T4.1-T4.5: `users.handoff_greeting_dispatched_at` DB column + atomic claim/clear/reset, `_handle_start_with_payload` schedules proactive greeting via FastAPI BackgroundTasks, `/tasks/retry-handoff-greetings` pg_cron backstop endpoint, one-shot stranded-user migration script, full `ClearanceGrantedCeremony` portal component. T4.6-T4.8 (retention cron, GDPR coupling, admin opt-in) deferred to follow-up PR. NR-1b (conversation history persistence). Technical spec at `specs/214-portal-onboarding-wizard/technical-spec.md`. Imports Spec 213 contracts. Supersedes 081. |
