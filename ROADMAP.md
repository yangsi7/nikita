...
| 218 | onboarding-wizard-v2-agent-driven | 0 (NEW) | **IN AUTHORING (2026-05-09)** — supersedes 217. Hybrid agent-driven dynamic-UI: Python router picks slot deterministically; Pydantic AI decorator agent emits 8-component discriminated-union envelope per turn (text_short, text_long, single_select, chip_multi, slider, calendar, phone, complete) decorated with reaction text folded into prompt; Phase 1 required slots + Phase 2 open-bounce 4-8 turns. Wow features: phone-demo opt-in via `voice_service.make_outbound_call` + Supabase Realtime + full-screen takeover. Static cohort lookup (NOT real-time firecrawl) for personalized hangouts. ~3000 LOC across 8 PRs (218-PREREQ-A backstory-timeout fix + 218-1..218-7). 25 architectural decisions locked in `~/.claude/plans/immutable-wondering-gray.md` brief; 8 reviewers verified (Pydantic AI research, Gemini deep research, process-auditor 31/40, devils-advocate, scope-reviewer, pattern-scout, codex, gemini reviewer). Mandatory bulldoze list per `.claude/rules/archive-policy.md` atomicity. Solo-dev no-backcompat per `feedback_solo_dev_no_backcompat_reinforced.md`. |

**Domain subtotal: 19 specs (15 complete, 4 active — Specs 216, 217 superseded-by-218, 218 NEW)**
