...
// Tests: "FIELD N OF 7" renders correctly with canonical letter-spaced
// uppercase chrome per `docs/content/wizard-copy.md` (Shared Chrome).

describe("WizardProgress", () => {
  it("renders the canonical 'FIELD N OF 7' label for the given step index", () => {
  ...
  it("applies the dossier letter-spacing + uppercase chrome classes", () => {
    render(<WizardProgress current={5} total={7} />)
    const el = screen.getByText("FIELD 5 OF 7")
    // Dossier field-label chrome per spec FR-2 / Appendix C reference
    ...
    // signifiers that make it visually a dossier label.
