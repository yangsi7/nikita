...
- **Auto-dispatch mandatory steps (HARD GATE)**: Workflow steps the rules already mandate (`/qa-review`, post-merge smoke tests, deployment verification, `/e2e`/dogfood runs, live HTTP probes, log-sweep verifications, GH issue close, ROADMAP sync, DB cleanup after dogfood) must auto-dispatch to a fresh subagent (or foreground action if short). Do NOT ask the user for permission; they are already authorized by the workflow. Two reasons: (a) keeps noisy review/test transcripts out of main context, (b) eliminates confirmation bias via fresh-context analysis. Spawn via `Agent` with a self-contained prompt (target PR/branch, success criteria, report shape, HARD CAP tool calls).
- **Pre-send self-check (HARD GATE)**: Before ending any turn, scan the draft response for banned offer-pattern strings: `"Want me to"`, `"Do you want to"`, `"Let me know if"`, `"You can also"`, `"Up to you"`, `"Shall I"`, `"or do you want to test it yourself"`. If any match AND the "offer" is a routine follow-through step (see above list), replace the offer with the action itself. The only legitimate asks are the 4 carve-outs in `~/.claude/rules/operating-principles.md` Core Behavior #8: (a) destructive ops, (b) scope expansion, (c) external-visible actions, (d) genuine spec ambiguity >1h rework cost. PR #312 (2026-04-17) precedent: "Want me to auto-drive the full dogfood walk, or do you want to test it yourself first?" after shipping onboarding wiring was the exact anti-pattern.
