...
Tests for T058-T061 acceptance criteria:
- AC-FR016-001: Ch1 user calls → stability=0.8, speed=0.95 (distant)
- AC-FR016-002: Ch5 user calls → stability=0.4-0.5 (expressive)
- AC-FR017-001: Nikita annoyed → speed=1.1, stability=0.4
- AC-FR017-002: Nikita vulnerable → speed=0.9, stability=0.7
"""
...
class TestTTSConfigChapterBased:
    ...
    def test_chapter_2_warming_voice(self):
        """Ch2 → slightly less stable, natural speed."""
        from nikita.agents.voice.tts_config import TTSConfigService
    ...
    def test_chapter_3_comfortable_voice(self):
        """Ch3 → balanced stability, normal speed."""
        from nikita.agents.voice.tts_config import TTSConfigService
    ...
    def test_chapter_4_intimate_voice(self):
        """Ch4 → lower stability for emotion, normal speed."""
        from nikita.agents.voice.tts_config import TTSConfigService
    ...
    def test_chapter_5_expressive_voice(self):
        ...
        assert 0.4 <= settings.stability <= 0.5
    ...
    def test_invalid_chapter_defaults_to_3(self):
        ...
        assert settings.stability == 0.6  # Ch3 default
        assert settings.speed == 1.0
...
class TestTTSConfigMoodBased:
    ...
    def test_flirty_mood_medium_expressive(self):
        """Flirty → moderate speed, lower stability for variation."""
        from nikita.agents.voice.tts_config import TTSConfigService
    ...
    def test_playful_mood_upbeat(self):
        """Playful → slightly faster, lower stability."""
        from nikita.agents.voice.tts_config import TTSConfigService
...
class TestTTSConfigCombined:
    ...
    def test_get_final_settings_mood_overrides_chapter(self):
        ...
        # Ch1 normally has stability=0.8, but annoyed mood overrides
        settings = service.get_final_settings(
            chapter=1, mood=NikitaMood.ANNOYED
        )

        # Mood takes priority
        assert settings.stability == 0.4  # Annoyed, not chapter 1
    ...
    def test_get_final_settings_no_mood_uses_chapter(self):
        ...
        assert settings.stability == 0.4  # Ch5 settings
        assert settings.speed == 1.02
    ...
    def test_get_final_settings_neutral_mood_uses_chapter(self):
        ...
        assert 0.4 <= settings.stability <= 0.6
        assert 1.0 <= settings.speed <= 1.02
