    ...
    switch (command) {
      ...
      case 'report': {
        ...
        for (const file of files) {
          ...
          // Match common test file patterns: __tests__, .test., .spec., or files starting with test- or ending with -test
          if (/__tests__|\.test\.|\.spec\.|\/test-|\/test\.|test\.js$|test\.ts$|test\.py$/i.test(file)) stats.tests++
        }
        // Count markdown documentation files from data.d
        if (data.d) {
          const docKeys = Object.keys(data.d)
          if (focus) {
            stats.docs = docKeys.filter(doc => doc.startsWith(focus)).length
          } else {
            stats.docs = docKeys.length
          }
        }
        // Hotspots
        const inboundCounts = []
